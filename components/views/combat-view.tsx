        ...
        {showWeapons && (
          <>
            ...
            {character.ranger_companion && (
              <div
                onClick={() => setActiveWeaponId(`companion-${character.ranger_companion.name}`)}
                ...
                <div className="p-4 flex justify-between items-start">
                  <div>
                    <div className="flex items-center gap-2">
                      ...
                      <h4 className="font-serif font-bold text-white text-lg">
                        {character.ranger_companion.name} - {character.ranger_companion.attack_name}
                      </h4>
                    </div>
                    <div className="flex gap-2 text-xs text-gray-400 mt-1">
                      <span className="uppercase bg-white/10 px-1.5 py-0.5 rounded">
                        {character.ranger_companion.attack_type}
                      </span>
                      <span className="uppercase bg-white/10 px-1.5 py-0.5 rounded">
                        {character.ranger_companion.attack_range?.replace('_', ' ')}
                      </span>
                  ...
                  <div className="text-right">
                    <div className="text-xl font-bold text-dagger-gold">
                      {calculateWeaponDamage(character.ranger_companion.damage_die, totalProficiency)}
                    </div>
                    <div className="text-[10px] text-gray-500 uppercase">
                      {character.ranger_companion.damage_die} × {totalProficiency}
                    </div>
                  </div>
                </div>
                ...
                <div className="bg-black/40 p-2 flex gap-2">
                  <button
                    onClick={(e) => {
                      ...
                      const attackModifier = calculateAttackModifier(character);
                      prepareRoll(`${character.ranger_companion.name} Attack`, attackModifier);
                    }}
                  ...
                  <button
                    onClick={(e) => {
                      e.stopPropagation();
                      const calculatedDamage = calculateWeaponDamage(character.ranger_companion.damage_die, totalProficiency);
                      const { dice, modifier } = parseDamageRoll(calculatedDamage);
                      const damageModifier = calculateDamageModifier(character);
                      const totalDamageBonus = modifier + damageModifier;
                      prepareRoll(`${character.ranger_companion.name} Damage`, totalDamageBonus, dice);
                    }}
                    className="flex-1 py-2 bg-white/10 hover:bg-white/20 rounded-lg text-sm font-bold flex items-center justify-center gap-2 transition-colors"
                  >
