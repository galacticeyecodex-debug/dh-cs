...
- **Iconography:** Use `AppIcons` from `@/lib/icon-utils` (size 14) placed before the text.
- **Toggle Button (Standard):**
  ...
  - **Icons:** `AppIcons.ui.visibility` and `AppIcons.ui.visibilityOff`.
...
<div className="flex items-center justify-between">
  <h3 className="text-xs font-bold uppercase text-gray-500 tracking-wider flex items-center gap-2">
    <AppIcons.combat.attack size={14} /> Active Weapons
  </h3>
  <button
    onClick={() => setShowWeapons(!showWeapons)}
    className="flex items-center gap-1 text-xs text-gray-500 hover:text-white transition-colors px-2 py-1 rounded"
  >
    {showWeapons ? <AppIcons.ui.visibilityOff size={14} /> : <AppIcons.ui.visibility size={14} />}
...
## 6. Icons (Source of Truth)

**`lib/icon-utils.ts` is the SINGLE SOURCE OF TRUTH for all icons in the application.**

This centralization:
- Ensures visual consistency across all views (Character, GM, Combat)
- Simplifies global refactoring (change icon in one place)
- Enables dynamic user customization for vital tracks via Settings
- Provides semantic mappings (`AppIcons`) for discoverable icon usage

> [!IMPORTANT]
> **NO DIRECT IMPORTS:** Components **MUST NOT** import from `lucide-react` directly. Always import from `@/lib/icon-utils`:
> - Use `AppIcons.category.name` for static icons
> - Use `getIconByName(vitalIcons.X, AppIcons.vitals.X)` for user-customizable vital icons
...
**Dynamic Vitals (User Preference):**
```tsx
...
### 6.2 Vital Icons Registry

| Vital | AppIcons Mapping | Fallback Icon | User Options |
|-------|-----------------|---------------|--------------|
| **Hit Points (HP)** | `AppIcons.vitals.hitPoints` | `Heart` | Heart, Flame |
| **Stress** | `AppIcons.vitals.stress` | `Zap` | Zap, Droplet |
| **Hope** | `AppIcons.vitals.hope` | `Sparkles` | Zap, Sparkles |
| **Armor** | `AppIcons.vitals.armor` | `Shield` | Shield, BrickWall |
| **Evasion** | `AppIcons.vitals.evasion` | `Eye` | Eye, Wind |
| **Fear** | `AppIcons.vitals.fear` | `Skull` | Skull, Moon |

> **IMPORTANT:** Users can customize vital icons via the Settings menu. The "Fallback Icon" column shows what `AppIcons.vitals.X` resolves to when no user preference is set. Components displaying vital icons **MUST** use the dynamic pattern to respect user preferences.

### 6.3 Common UI & Combat Registry

| Category | Mapping | Purpose |
|----------|---------|---------|
| **UI** | `AppIcons.ui.settings` | Account/Campaign settings |
| **UI** | `AppIcons.ui.delete` | Destructive removal |
| **UI** | `AppIcons.ui.edit` | Modifying content |
| **UI** | `AppIcons.ui.search` | Search inputs |
| **Campaign** | `AppIcons.campaign.gm` | Game Master badge |
| **Campaign** | `AppIcons.campaign.party` | Party members view |
| **Combat** | `AppIcons.combat.roll` | Dice roll actions |
| **Combat** | `AppIcons.combat.attack` | Attack/Weapon indicator |

### 6.4 Icon Sizing Guidelines

| Context | Size | Example |
|---------|------|---------|
| **Section Headers** | `size={14}` | Compact label icons |
| **Buttons/Actions** | `size={16}` | Inline action buttons |
| **Card Headers** | `size={18}` or `size={20}` | Prominent feature icons |
| **Large Indicators** | `size={24}` or greater | Hero/splash icons |

### 6.5 Icon Fill Patterns for Vitals

When displaying filled vs unfilled icons (e.g., icon tracks):

**Standard Pattern:**
```tsx
<Icon
    fill={isFilled ? "currentColor" : "none"}
    className={isFilled ? vital.color : "text-white/10"}
/>
```

**High Contrast (for skulls/fear where fill obscures stroke):**
```tsx
<Skull
...
/>
```
This uses explicit fill colors with darker stroke colors to ensure the icon lines remain visible when filled.
