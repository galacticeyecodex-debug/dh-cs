/**
 * PUBLIC CONTENT PARSER & SQL GENERATOR
 * ----------------------------------------------------------------------------
 * This script transforms public SRD JSON data into a SQL seed file.
 *
 * FUNCTIONALITY:
 * - Reads JSON files from `content/public/srd/json`
...
// Main execution
try {
  console.log('Starting public content parse...');
  console.log(`SRD JSON directory: ${SRD_JSON_DIR}`);
  console.log(`Output path: ${OUTPUT_PATH}`);
  console.log('');

  // Process SRD content only (private content has its own seed script)
  if (!fs.existsSync(SRD_JSON_DIR)) {
    console.error(`SRD directory not found: ${SRD_JSON_DIR}`);
    process.exit(1);
  }

  console.log('Processing SRD content...');
  processClasses(SRD_JSON_DIR, 'srd');
  processSubclasses(SRD_JSON_DIR, 'srd');
  processAncestries(SRD_JSON_DIR, 'srd');
  processCommunities(SRD_JSON_DIR, 'srd');
  processDomains(SRD_JSON_DIR, 'srd');
  processAbilities(SRD_JSON_DIR, 'srd');
  processWeapons(SRD_JSON_DIR, 'srd');
  processArmor(SRD_JSON_DIR, 'srd');
  processConsumables(SRD_JSON_DIR, 'srd');
  processAdversaries(SRD_JSON_DIR, 'srd');
  processItems(SRD_JSON_DIR, 'srd');
  processEnvironments(SRD_JSON_DIR, 'srd');
  processTransformations(SRD_JSON_DIR, 'srd');
  linkSubclassesToClasses(SRD_JSON_DIR);

  const header = `-- Daggerheart Library Seed Data (Public SRD Content)
-- Generated by content/public/seed-public-library.js
